...
<body>
	<ul class="nav nav-tabs">
		...
		<li class="active"><a href="${ctx}/rpt/core/rptCm/form?id=${rptCm.id}">报表模板发布<shiro:hasPermission name="rpt:core:rptCm:edit">${not empty rptCm.id?'修改':'添加'}</shiro:hasPermission><shiro:lacksPermission name="rpt:core:rptCm:edit">查看</shiro:lacksPermission></a></li>
	</ul><br/>
	<form:form id="inputForm" modelAttribute="rptCm" action="${ctx}/rpt/core/rptCm/publishSave" method="post" class="form-horizontal">
		<form:hidden path="id"/>
		...
		<div class="control-group">
			<label class="control-label">发布说明：</label>
			<div class="controls">
				<form:textarea path="releaseRemark" htmlEscape="false" rows="4" class="input-xxlarge " maxlength="255"/>
			</div>
		</div>
