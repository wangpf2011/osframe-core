	...
	<script type="text/javascript">
		...
		function doUpLoadFile(){
			//form上传的URL 业务地址-自定义修改
           	var upUrl='${ctx}/rpt/core/rptCm/regupload';
           	
           	//弹出PopUP
    		top.$.jBox.open("iframe:${ctx}/sys/log/uploadPopup?upUrl="+upUrl, "上传模板文件", 500, $(top.document).height()-400, {
                buttons:{"确定":"ok", "关闭":true}, submit:function(v, h, f){
                    if (v=="ok"){
                    	//获得弹出页面隐藏域值
	                    var fileName= h.find("iframe")[0].contentWindow.$("#fileName").val();
	                    var filePath= h.find("iframe")[0].contentWindow.$("#filePath").val();
	                    var relfileName= h.find("iframe")[0].contentWindow.$("#relfileName").val();
	                    //本页面赋值
				        $("#filePath").val(filePath);
				        $("#fileName").val(fileName);
				        $("#relfileName").val(relfileName);
                    }else if (v=="clear"){
                    }
                }, loaded:function(h){
...
<body>
	<ul class="nav nav-tabs">
		...
		<li class="active"><a href="${ctx}/rpt/core/rptCm/form?id=${rptCm.id}">报表模板<shiro:hasPermission name="rpt:core:rptCm:edit">${not empty rptCm.id?'修改':'添加'}</shiro:hasPermission><shiro:lacksPermission name="rpt:core:rptCm:edit">查看</shiro:lacksPermission></a></li>
	</ul><br/>
	<form:form id="inputForm" modelAttribute="rptCm" action="${ctx}/rpt/core/rptCm/save" method="post" class="form-horizontal">
		...
		<div id="div1" class="control-group">
			...
			<div class="controls">
				<form:input path="pageName" htmlEscape="false" class="input-xlarge "/>
			</div>
		</div>
		<div class="control-group">
		...
		<div class="form-actions">
			<shiro:hasPermission name="rpt:core:rptCm:edit">
